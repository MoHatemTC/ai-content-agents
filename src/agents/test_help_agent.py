...
class TestHelpAgent:
    ...
    def generate(
        ...
        if self.mock_mode:
            ...
            if question_type.value == "short_answer":
                ...
                stem = "Name the two loop types provided by Python."
                ...
                answer = "while"
                stem = "Which loop type is provided by Python?"
            questions = [
                {
        ...
        try:
            result = TestHelpOutput.model_validate(response_json)
    ...
    def generate_reviewable(
        ...
        """Generate Test Help content as a pending human-review record."""
        agent_run = AgentRun(
    ...
    ) -> BatchGenerationResult[TestHelpOutput]:
        """Generate practice tests sequentially while retaining item failures."""
        started_at = perf_counter()
        successful_outputs: list[TestHelpOutput] = []
